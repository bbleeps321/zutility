object AboutBox: TAboutBox
  Left = 200
  Top = 108
  BorderStyle = bsDialog
  Caption = 'About'
  ClientHeight = 213
  ClientWidth = 298
  Color = clBlack
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object OKButton: TButton
    Left = 111
    Top = 180
    Width = 75
    Height = 25
    Caption = 'OK'
    Default = True
    ModalResult = 1
    TabOrder = 0
    OnClick = OKButtonClick
  end
  object PageControl: TPageControl
    Left = 8
    Top = 8
    Width = 281
    Height = 161
    ActivePage = ZPlayerSheet
    TabOrder = 1
    object ZUtilitySheet: TTabSheet
      Caption = 'ZUtility'
      object Comments: TLabel
        Left = 8
        Top = 104
        Width = 239
        Height = 26
        Caption = 
          'ZUtility provides a tool window for using simple programs rangin' +
          'g from music playing to text editing.'
        WordWrap = True
        IsControl = True
      end
      object Copyright: TLabel
        Left = 8
        Top = 80
        Width = 138
        Height = 13
        Caption = 'Copyright: 2007 Carey Zhang'
        IsControl = True
      end
      object ProgramIcon: TImage
        Left = 8
        Top = 8
        Width = 65
        Height = 57
        Picture.Data = {
          
          0000}
        Stretch = True
        IsControl = True
      end
      object Version: TLabel
        Left = 88
        Top = 40
        Width = 24
        Height = 13
        Caption = '1.0.0'
        IsControl = True
      end
      object ProductName: TLabel
        Left = 88
        Top = 16
        Width = 32
        Height = 13
        Caption = 'ZUtility'
        IsControl = True
      end
    end
    object ZPlayerSheet: TTabSheet
      Caption = 'ZPlayer'
      ImageIndex = 1
      object Label1: TLabel
        Left = 8
        Top = 104
        Width = 216
        Height = 26
        Caption = 'ZPlayer is a simple media player that provides basic functions.'
        WordWrap = True
        IsControl = True
      end
      object Label2: TLabel
        Left = 8
        Top = 80
        Width = 138
        Height = 13
        Caption = 'Copyright: 2007 Carey Zhang'
        IsControl = True
      end
      object Image1: TImage
        Left = 8
        Top = 8
        Width = 65
        Height = 57
        Picture.Data = {
          
          0000}
        Stretch = True
        IsControl = True
      end
      object Label3: TLabel
        Left = 88
        Top = 40
        Width = 24
        Height = 13
        Caption = '1.0.0'
        IsControl = True
      end
      object Label4: TLabel
        Left = 88
        Top = 16
        Width = 36
        Height = 13
        Caption = 'ZPlayer'
        IsControl = True
      end
    end
  end
end
